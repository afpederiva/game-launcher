...
export default function App() {
  ...
  const handleUpdateStatus = async (game, status) => {
    ...
    // Update selected game if it's the same
    if (selectedGame?.romPath === game.romPath) {
      setSelectedGame({ ...game, status });
    }
  };

  // Rename game
  const handleRenameGame = async (game) => {
    const newName = prompt('Enter new name:', getGameDisplayName(game));
    if (!newName || !window.launcher) return;
    ...
    updateGameInLibrary(game, (g) => {
      g.customName = newName;
    });

    if (selectedGame?.romPath === game.romPath) {
      setSelectedGame({ ...game, customName: newName });
    }
  };
